...
class ActivityLogEntry {
  ...
  factory ActivityLogEntry.fromJson(Map<String, dynamic> json) {
    return ActivityLogEntry(
      ...
      progressDelta: json['progressDelta'] is int ? json['progressDelta'] as int : 1,
      timestamp: json['timestamp'] != null
          ? DateTime.tryParse(json['timestamp'].toString())?.toUtc() ?? DateTime.now().toUtc()
          : DateTime.now().toUtc(),
    );
  }
...
class StreakCalculator {
  ...
  static int calculateCurrentStreak(List<ActivityLogEntry> logs, {DateTime? now}) {
