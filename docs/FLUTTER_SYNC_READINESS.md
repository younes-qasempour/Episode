# Episode Flutter Sync Integration Status (Phase 2 Completed)
...
1. **Backend API Integration**: Integrated with the companion backend REST
   endpoints (`/auth/register`, `/auth/login`, `/auth/refresh`, `/auth/logout`,
   `/auth/logout-all`, `/users/me`, `/devices`, `/sync/status`, `/sync/pull`,
   `/sync/push`). The repository/deployment slug `otakulog-backend` is retained
   as a stable legacy identifier; it is not the current product name.
...
3. **Client Device Identity**: Client-generated persistent UUID v4 device ID
   stored in `SharedPreferences` key `otaku_log_client_device_id_v1`. This is a
   stable legacy compatibility key and must not be renamed without migration.
