...
void main() {
  group('Smart Collections Filtering Logic', () {
    ...
    test('filters Binge Worthy items correctly', () {
      final bingeWorthy = items.where((i) => i.tags.contains('Binge Worthy') || i.currentProgress > 10).toList();
      expect(bingeWorthy.length, 2);
    });

    test('filters On-Going items correctly', () {
      final ongoing = items.where((i) => i.releaseStatus == ReleaseStatus.ongoing).toList();
